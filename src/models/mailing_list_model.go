...
func CreateMailingList(mailingListName string, businessID string, db *sql.DB) (MailingList, error) {
...
func GetMailingList(mailingListID string, db *sql.DB, c *gin.Context) (MailingList, []Customer, error) {
	var thisMailingList MailingList
	var thisCustomers []Customer
	var thisCustomersID []int

	// Get information from this mailingLit
	mailingListSQL := `SELECT * FROM campaign WHERE mailing_list_id=$1;`
	...
	customerSQL := `SELECT * FROM customers WHERE `

	for i := 0; i < len(thisCustomersID); i++ {
		if i != 0 {
			customerSQL += `OR`
		}
		customerSQL += `customer_id=` + strconv.Itoa(thisCustomersID[i])
	}
